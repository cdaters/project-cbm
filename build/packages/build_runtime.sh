...
out="$w/packages/poc4-final"
mkdir "$out"
mkdir "$out/product"
tar -xf "$w/inputs/project-cbm-integration-poc4-final.tar" -C "$out/product"
...
for name in runtime menu; do
  tar --sort=name --mtime="@$SOURCE_DATE_EPOCH" --owner=0 --group=0 --numeric-owner --anchored --exclude="$name/debian" -C "$out" -cf - "$name" | gzip -n > "$out/project-cbm-${name}_1.1.0~poc4.orig.tar.gz"
