# POC4 corrective attempt #6 — exact-hash Pi 3B regression
...
Codex performed no physical test. [Build report](../build/private-poc4-attempt6.md) and
[exact result](../build/private-poc4-attempt6.json) record the validation and limits.

| Identity | Exact binding |
| --- | --- |
| Product | 1.1.0-poc.4 / private-engineering-poc4, corrective attempt 6 |
| Integration | `5992e2a7b1a84cbad016a293624d48aaa7cdd1cf` |
| Menu | `v1.1.0_poc4.4`, peeled `6522111691ce494b00637549365d6a5f2a7773ff` |
| Lock | `inputs/frozen-poc4-attempt6/release-lock.json`, schema 4 |
| Lock SHA-256 | `8a5f7c929e73ac38ab42945b2bf22d577eb2b8a9e0500dd69c27f005f75c6de2` |
| Raw | `artifacts/private-poc4-attempt-6/2026-09-18-project-cbm-1.1.0-poc.4-lite-private-poc.img`, 3087007744 bytes |
| Raw SHA-256 | `bf81b1f95ab65d19753f85fdafbb5a93f45eae68b0bea4208bef62c9d6a2c328` |
| XZ | `artifacts/private-poc4-attempt-6/image_2026-09-18-project-cbm-1.1.0-poc.4-lite-private-poc.img.xz`, 598195324 bytes |
...
`/Volumes/TheBench/ProjectCBM-Work`. Recovery: `archive/poc4-attempt6-2026-09-18`.
The manifest/restore report binds final source refs, exact inputs and artifacts.

Attempt #5's owner report remains unchanged: lifecycle and information IP visibility
PASS, visible Cover and initial scan FAIL, other unreported behavior UNTESTED.
This candidate addresses the observed Cover budget/dwell defect, waits for completed
Wi-Fi scans, masks password input, uses ASCII ranges and adds authoritative Main Menu
IP information. The six-second Cover budget is a qualification limit, not measured Pi
startup latency. Submission telemetry does not establish visible artwork.
...
28. On the first boot, after region/regulatory setup, the initial Wi-Fi scan with no
    Ethernet cable finds an owner-controlled WPA-personal AP without manually choosing
    another scan. Note immediate working feedback and actual elapsed time. If readiness
    or completion cannot be confirmed within the bounded operation, verify a truthful
    retry/offline result instead of a misleading successful empty list. Preserve the
    fixed scan outcome below; do not retry repeatedly before saving failure evidence.
29. SSID selection; no unsupported/open/enterprise network claim.
30. Owner and Wi-Fi password fields show one masking mark per typed character.
    No plaintext appears. Owner guidance reads `12-128 printable characters`; Wi-Fi
    guidance reads `8-63 printable ASCII characters`, without escape/formatting garbage.
    Do not photograph entered secrets or include values in reports.
...
51a. Main Menu shows concise current IP information after each redraw. Exercise no
    connected interface, one Ethernet connection, one Wi-Fi connection and both together
    when available. Compare with Network Information; do not assume eth0/wlan0 names.
    Multiple links must remain concise, with detailed IP/MAC data in the information
    views. No credentials appear. Measure Menu return/redraw latency on Pi 3B and note
    any regression from attempt #5. Unsupported/unavailable equipment stays UNTESTED.
...
- `initializing`, `video`, `decoder_init`, `window_create`, `renderer_create`,
  `renderer`, `texture_load`, `present_begin`, `presented`, `releasing`, `released`:
  successive SDL stages with elapsed milliseconds. Record whole-process timeout,
  exit/signal and phase timing; normal completion does not wait for the six-second cap.
...
out=$(mktemp -d /home/pi/pcbm/programs/Qualification/poc4-attempt6-evidence.XXXXXXXX) || exit 1
mkdir "$out/automatic"
...
if [ -f /var/lib/project-cbm/setup/wifi-scan.json ]; then
  cp /var/lib/project-cbm/setup/wifi-scan.json "$out/wifi-scan.json"
fi
pcbm-diagnostics
