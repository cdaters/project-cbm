...
class RC3(unittest.TestCase):
 def test_current_filename_and_candidate_are_coherent(self):
  verify_output_identity(ROOT,read_json(ROOT/'build/pigen/config.json'),'1.1.0-rc.4','private-engineering-rc4')
 def test_stale_export_suffix_is_rejected_before_freeze(self):
  with tempfile.TemporaryDirectory() as temp:
   ...
   with self.assertRaises(ValueError):verify_output_identity(root,{'image_name':'project-cbm-1.1.0-rc.4'},'1.1.0-rc.4','private-engineering-rc4')
 def test_single_user_contract_applies_to_rc3_validators(self):
  for name in ('validate_activation.py','validate_poc_image.py','validate_poc4_supplement.py'):
   self.assertIn("('private-engineering-rc2','private-engineering-rc3','private-engineering-rc4')",(ROOT/'tools'/name).read_text())
