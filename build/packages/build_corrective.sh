...
out="$w/packages/poc4-attempt4"
mkdir "$out"
mkdir "$out/product"
tar -xf "$w/inputs/project-cbm-integration-poc4-attempt4.tar" -C "$out/product"
mv "$out/product/project-cbm" "$out/runtime"
rmdir "$out/product"
tar -xf "$w/inputs/project-cbm-menu-1.1.0_poc4.2.tar" -C "$out"
mv "$out/project-cbm-menu-1.1.0_poc4.2" "$out/menu"
