#!/usr/bin/env python3
"""Freeze corrective attempt 5; retain attempt 4 and all unchanged component bytes."""
import argparse
import copy
import hashlib
import os
...
def main():
    ...
    integration = w/'inputs/project-cbm-integration-poc4-attempt5.tar'
    ...
    packages = w/'packages/poc4-attempt5'
    oldrecord = read_json(old/lock['components']['menu']['build_record']['path'])
    for name, version, source, commit, ref, tag in (
        ('runtime', '1.1.0~poc4.2-1', integration, a.integration_commit,
         'refs/heads/feature/1.1-build-foundation', None),
        ('menu', '1.1.0~poc4.3-1+pcbm1', w/'inputs/project-cbm-menu-1.1.0_poc4.3.tar',
         a.menu_commit, 'refs/tags/v1.1.0_poc4.3', a.menu_tag_object)):
        ...
        component['version'] = '1.1.0_poc4.2' if name == 'runtime' else '1.1.0_poc4.3'
    ...
    (kit/'attempt.json').write_bytes(encode({'attempt':5,'candidate':lock['product'],
        ...
        'construction_argument':'--attempt 5'}))
    print(sha)
