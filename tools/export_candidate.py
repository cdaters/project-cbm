...
def main():
    ...
    if mc!=git(menu,'rev-parse','HEAD') or git(menu,'cat-file','-t',a.menu_tag)!='tag':raise ValueError('Menu must be at exact annotated tag')
    descriptors=[]
    for repo,ref,prefix,name in [(root,commit,'project-cbm/',f'project-cbm-integration-poc4-attempt{a.attempt}.tar'),(menu,mc,'project-cbm-menu-'+a.menu_tag[1:]+'/','project-cbm-menu-'+a.menu_tag[1:]+'.tar')]:
        path=w/'inputs'/name
        with path.open('xb') as stream:subprocess.run(['git','-C',str(repo),'archive','--format=tar','--prefix='+prefix,ref],stdout=stream,check=True)
