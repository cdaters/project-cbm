#!/usr/bin/env python3
"""Freeze corrective attempt 6; retain attempt 5 and all unchanged component bytes."""
import argparse
import copy
import hashlib
import os
...
def main():
    ...
    integration = w/'inputs/project-cbm-integration-poc4-attempt6.tar'
    ...
    packages = w/'packages/poc4-attempt6'
    oldrecord = read_json(old/lock['components']['menu']['build_record']['path'])
    for name, version, source, commit, ref, tag in (
        ('runtime', '1.1.0~poc4.3-1', integration, a.integration_commit,
         'refs/heads/feature/1.1-build-foundation', None),
        ('menu', '1.1.0~poc4.4-1+pcbm1', w/'inputs/project-cbm-menu-1.1.0_poc4.4.tar',
         a.menu_commit, 'refs/tags/v1.1.0_poc4.4', a.menu_tag_object)):
        ...
        component['version'] = '1.1.0_poc4.3' if name == 'runtime' else '1.1.0_poc4.4'
    ...
    for source, dest in [('lib/pcbm_info_view.py','usr/libexec/project-cbm-menu/pcbm_info_view.py'),
    ...
    (kit/'attempt.json').write_bytes(encode({'attempt':6,'candidate':lock['product'],
        ...
        'changed':['runtime package/source','Menu package/tag/source','integration source/commit',
                   'bounded Cover timing','first-boot UX','builder capacity configuration','corrective validation records','installed identity'],
        ...
        'construction_argument':'--attempt 6'}))
    print(sha)
